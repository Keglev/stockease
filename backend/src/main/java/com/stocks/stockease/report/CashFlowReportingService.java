...
/**
 * Money in and out, on a payment basis, grouped either by product or by calendar month.
 *
 * <p>An invoice counts on the date it was paid, never on the date it was booked (ADR 025), so a
 * booked-but-unpaid invoice is invisible here whatever the period. That is the whole difference from
 * {@link ProfitReportingService}: purchases and supplier returns move cash and appear here, while
 * profit ignores them (ADR 024).
 *
 * <p>The two reports answer the same question at different groupings and share their SQL fragments
 * for that reason - a timeline month and the product rows underneath it must never sum to different
 * figures, and a forked query is how they would start to.
 */
@Service
@RequiredArgsConstructor
@Transactional(readOnly = true)
public class CashFlowReportingService {
     ...
     * @param from first payment date to count, or {@code null} for no lower bound
     * @param to last payment date to count, or {@code null} for no upper bound
     * @param productId narrows the report to one product's lines, or {@code null} for all products
