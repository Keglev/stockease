...
/** Slice tests for DELETE /api/suppliers/{id}, covering the admin-only rule and the open-invoice veto. */
@ExtendWith(MockitoExtension.class)
@WebMvcTest(SupplierController.class)
@Import({TestConfig.class, SupplierMethodSecurityTestConfig.class})
class SupplierDeleteControllerTest {
    ...
    void deleteSupplier_asAdmin_returns200() throws Exception {
        mockMvc.perform(delete("/api/suppliers/1").with(csrfToken()))
                ...
                .andExpect(jsonPath("$.message").value("Supplier with ID 1 has been successfully deleted."));

        Mockito.verify(supplierService, Mockito.times(1)).deleteById(1L);
    }

    @Test
    @WithMockUser(username = "user", roles = {"USER"})
    void deleteSupplier_asUserRole_returns403() throws Exception {
        mockMvc.perform(delete("/api/suppliers/1").with(csrfToken()))
                .andExpect(status().isForbidden());

        Mockito.verify(supplierService, Mockito.never()).deleteById(anyLong());
    }

    @Test
    void deleteSupplier_asAnonymous_returns401() throws Exception {
        mockMvc.perform(delete("/api/suppliers/1").with(csrfToken()))
    ...
    @Test
    @WithMockUser(username = "admin", roles = {"ADMIN"})
    void deleteSupplier_withMissingId_returns404() throws Exception {
        Mockito.doThrow(new EntityNotFoundException("Supplier with ID 9 not found."))
                .when(supplierService).deleteById(9L);

        mockMvc.perform(delete("/api/suppliers/9").with(csrfToken()))
    ...
    void deleteSupplier_withOpenInvoices_returns409() throws Exception {
        ...
        mockMvc.perform(delete("/api/suppliers/1").with(csrfToken()))
                ...
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("Cannot delete supplier 'Acme': open invoices exist."))
