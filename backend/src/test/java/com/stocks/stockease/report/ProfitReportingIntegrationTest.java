...
import static com.stocks.stockease.report.ReportingTestFixtures.firstItemId;
...
/*
 * Contract: the two profit reports - per product and per supplier - read against data built
 * through the domain services.
 *
 * What is specified here is which units count and at which price. Revenue and cost follow the
 * units that stayed sold, valued at what the sale captured rather than at today's catalogue
 * price (ADR 019, ADR 024), so returns in each direction land differently: a customer return
 * reverses both sides, while sending stock back to a supplier moves cash and must leave profit
 * untouched. Stock still on the shelf is inventory, never a cost of goods sold.
 *
 * The date-window cases pin a structural choice rather than a number: the date test lives in the
 * JOIN, so a product or supplier with nothing booked in the window keeps a zero row instead of
 ...
 *
 * Out of scope: stock levels and losses (StockStatusAndLossReportingIntegrationTest) and the
 * unpaid-invoice reports (InvoiceAgingReportingIntegrationTest).
 */
@SpringBootTest
@ActiveProfiles("test")
class ProfitReportingIntegrationTest extends AbstractIntegrationTest {
    ...
    private Invoice closedPurchase(long supplierId, long productId, int qty, String unitPrice) {
        return fixtures.closedPurchase(supplierId, productId, qty, unitPrice);
    }

    private Invoice closedSale(long productId, int qty, String unitPrice) {
        return fixtures.closedSale(productId, qty, unitPrice);
    }

    private void record(MovementReason reason, long productId, int qty, Long itemId) {
        fixtures.record(reason, productId, qty, itemId);
    }
