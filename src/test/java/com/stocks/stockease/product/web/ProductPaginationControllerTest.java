...
class ProductPaginationControllerTest {
    ...
    void getProductsPaged_withValidParams_returns200(String username, String role) throws Exception {
    ...
    @ParameterizedTest
    @CsvSource({"adminUser, ADMIN", "regularUser, USER"})
    void getProductsPaged_withNegativeParams_returns400(String username, String role) throws Exception {
        mockMvc.perform(get("/api/products/paged")
                        .with(userWithRole(username, role))
                        .param("page", "-1")
                        .param("size", "-10"))
